   ...
   public function ToString() {
     ...
     $Session = Gdn::Session();
      ...
      <h4><?= T('Голосование в теме')?></h4> 
       ...
       <br />
       <div class="poll-bar" style="width:<?= floor(($Answer->votes / $this->_PollVotes) * 100) + 1?>%;"></div> <span class="poll-percentage"><?= $Percentage?>% </span><span class="poll-numvotes">(<?= $Answer->votes?> голосов)</span>
       <br />
       </li>
       <?endforeach?>
       </ul>
      ...
      <li><?= Anchor(T('Редактировать голосование'), 'vanilla/discussion/editpoll/'.$this->_PollData->id, 'Reset poll')?></li><li><?= Anchor(T('Сбросить голосование'), 'vanilla/discussion/poll/'.$this->_PollData->id.'/reset', 'Reset poll')?></li><li><?= Anchor(T('Удалить голосование'), 'vanilla/discussion/poll/'.$this->_PollData->id.'/delete', 'Delete poll')?></li>
       </ul>
       <?endif?>
        <?else:?>
        <?if($Session->CheckPermission('Plugins.Poll.Manage')):?>        
           <ul class="poll-options-list">
        <li><?= Anchor(T('Добавить голосование'), 'vanilla/discussion/attachpoll/'.$this->ID, 'Attach a poll')?></li>
       </ul>
        <?else:?>
        <?= T('Нет голосований в теме') ?>
        <?endif?>
        <?endif?>
      </div>
      <?php
