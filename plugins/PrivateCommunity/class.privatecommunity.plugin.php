...
class PrivateCommunityPlugin extends Gdn_Plugin {
   ...
   public function RoleController_AfterRolesInfo_Handler($Sender) {
      ...
      if ($Private) {
         echo Wrap('Your community is currently <strong>PRIVATE</strong>.', 'span', array('style' => 'background: #ff0; padding: 2px 4px;'));
         echo Wrap(Anchor('Switch to PUBLIC', 'settings/privatecommunity/on/'.Gdn::Session()->TransientKey(), 'SmallButton').' - All visitors will be able to see the content in your community.', 'div');
      } else {
         echo Wrap('Your community is currently <strong>PUBLIC</strong>.', 'span', array('style' => 'background: #ff0; padding: 2px 4px;'));
         echo Wrap(Anchor('Switch to PRIVATE', 'settings/privatecommunity/off/'.Gdn::Session()->TransientKey(), 'SmallButton').' - Only signed-in users will be able to see the content in your community.', 'div');
      }
      echo '</div>';
   }
