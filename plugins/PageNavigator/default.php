...
// You can pm me at vanillaforums, if you like the plugin and care to donate $$ to the cause.
...
class PageNavigatorPlugin extends Gdn_Plugin {
    ...
    public function PluginController_PageNavigator_Create($Sender) {
  ...
  //   echo sprintf(' <a class="pagenavigator" href="discussion/'  . $did  ."/" . $dtitle . "/#Item_1" . '">First</a>');
         echo sprintf(' <a class="pagenavigator pagen" href="index.php?p=/discussion/'  . $did . '/x/p1' . '">' . $First .  '</a>');
     
          }
         if (C('Plugins.PageNavigator.Show_Last')) {         
      //     echo sprintf(' <a class="pagenavigator" href="discussion/'  . $did  ."/" . $dtitle . "/#Item_" . $itemlast . '">Last</a>');  
         // alternate
          echo sprintf(' <a class="pagenavigator pagen"  href="index.php?p=discussion/comment/' . $lid . '#Comment_' . $lid . '">' . $Last .  '</a>');
    ...
    public function DiscussionController_BeforeCommentMeta_Handler($Sender) {
     if ((C('Plugins.PageNavigator.Show_Top')) ||  (C('Plugins.PageNavigator.Show_Bottom')) ){
        ...
        if ($topcom != $botcom) {
          ...
          echo "<span class=pagens>";
            echo sprintf('<a class="pagenavigator pntop" >' . $Up . ' </a>');
           }
           ...
           // echo sprintf(' <a class="pagenavigator" href="/vanilla/index.php?p=/discussion/comment/' . $botcom . '#Comment_' . $botcom . '">Bottom</a>');
            ...
            echo sprintf('<a class="pagenavigator pnbottom" > ' . $Down . '</a></span>');
            echo "</span>";
            }
        }
     }
