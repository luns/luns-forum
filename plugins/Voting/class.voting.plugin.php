...
//			return;
   ...
   public function CategoriesController_Render_Before($Sender) {
	 ...
	 */
	public function Base_DiscussionMetaLuns_Handler($Sender) {
//		if (!C('Plugins.Voting.Enabled'))
//			return;
...
//			return;
		...
		echo '<span class="Votes">';
			...
			echo Anchor('<i class="icon icon-thumbs-up"></i> '.Wrap(Wrap('', 'i'), 'i', array('class' => '', 'rel' => 'nofollow')), $VoteUpUrl, '');
			echo '<span class="">'.Wrap(StringIsNullOrEmpty($Object->Score) ? '0' : Gdn_Format::BigNumber($Object->Score)).'</span>';
			echo Anchor('<i class="icon icon-thumbs-down"></i> '.Wrap(Wrap('', 'i'), 'i', array('class' => '', 'rel' => 'nofollow')), $VoteDownUrl, '');
		}
                        echo '</span>';
	}
...
//			return;
		...
		echo '<li'.($Sender->RequestMethod == 'popular' ? ' class="Active"' : '').'>'
			.Anchor(T('Popular'), '/discussions/popular', '')
		.'</li>';
	}
