...
//      $Sender->Validation->ApplyRule('Type', 'Required', T('Choose whether you want to ask a question or start a discussion.'));
   ...
   public function PostController_BeforeFormInputs_Handler($Sender, $Args) {
      $Sender->AddDefinition('QuestionTitle', T('Заголовок вопроса'));
      $Sender->AddDefinition('DiscussionTitle', T('Заголовок темы'));
      $Sender->AddDefinition('QuestionButton', T('Как вопрос'));
      $Sender->AddDefinition('DiscussionButton', T('Создать тему'));
      ...
      if ($Sender->Form->GetValue('Type') == 'Question' && $QuestionButton) {
         Gdn::Locale()->SetTranslation('Discussion Title', T('Заголовок ответа'));
         Gdn::Locale()->SetTranslation('Post Discussion', T('Как вопрос'));
      }
   ...
   public function PostController_Render_Before($Sender, $Args) {
      ...
      if ($Form->GetValue('Type') == 'Question' && $QuestionButton) {
         $Sender->SetData('Title', T('Как вопрос'));
      }
   }
