    ...
    public function ToString() {
        ...
        <div class="Box">
            <ul class="nav nav-list">
                ...
                <?php echo T($this->_DiscussionID > 0 ? 'Tagged' : 'Popular Tags'); ?>
                </li>
                <li class="divider"></li>
                <li>
                <?php
                    ...
                    if ($Tag->Name != '') {
                        ...
                        <?php
                            if (urlencode($Tag->Name) == $Tag->Name) {
                                echo Anchor('<span class="label label-info">' . htmlspecialchars($Tag->Name) . ': ' . number_format($Tag->CountDiscussions) . '</span>', 'discussions/tagged/' . urlencode($Tag->Name));
                            } else {
                                echo Anchor('<span class="label label-info">' . htmlspecialchars($Tag->Name) . ': ' . number_format($Tag->CountDiscussions) . '</span>', 'discussions/tagged?Tag=' . urlencode($Tag->Name));
                            }
                            ?>
                        <?php
                    }
