...
<?php
	...
	if($Session->CheckPermission('Plugins.Van2Shout.Post')) {
		...
		echo " <img src='/".Gdn::Request()->Webroot()."/applications/dashboard/design/images/progress.gif' style='display:none;' id='shoutboxloading' />\n";
		...
		$.get(gdn.url('plugin/Van2ShoutData?postcount=50'), function(data) {
			...
			for(var key in array) {
				...
				if(unparsed.indexOf('[!pmcontent!]') != -1) {
					...
					string = string + "<li>" + DeleteMsg(id[0]) + "<font color='" + timecolour + "'>[" + time + "]</font> <strong id='post" + id[0] + "'>PM from <a href='" + gdn.url('profile/' + parsedArray[0]) + "' target='blank' >" + parsedArray[0] + "</a>: " + idArray[0] + "</strong></li>";
				} else if (unparsed.indexOf('[!pmtocontent!]') != -1) {
					var parsedArray = unparsed.split("[!pmtocontent!]");
					var idArray = parsedArray[1].split("[!msgid!]");
					var id = idArray[1].split("[!msgtime!]");
					var time = moment.unix(id[1]).calendar();
					string = string + "<li>" + DeleteMsg(id[0]) + "<font color='" + timecolour + "'>[" + time + "]</font> <strong class='pmto'>PM to <a href='" + gdn.url('profile/' + parsedArray[0]) + "' target='blank' >" + parsedArray[0] + "</a>: " + idArray[0] + "</strong></li>";
				} else {
					var parsedArray = unparsed.split("[!content!]");
					var idArray = parsedArray[1].split("[!msgid!]");
					var id = idArray[1].split("[!msgtime!]");
					var time = moment.unix(id[1]).calendar();
					string = string + "<li>" + DeleteMsg(id[0]) + "<font color='" + timecolour + "'>[" + time + "]</font> <strong id='post" + id[0] + "'><a href='" + gdn.url('profile/' + parsedArray[0]) + "' target='blank' >" + parsedArray[0] + "</a>: " + idArray[0] + "</strong></li>";
				}
