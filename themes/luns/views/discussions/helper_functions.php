...
function WriteDiscussion($Discussion, &$Sender, &$Session, $Alt2) {
    ...
    echo ('<tr class="well well-small" style="background-color: #fafafa;">');
    echo ('<td>');
...
function WriteOptions($Discussion, &$Sender, &$Session) {
    if ($Session->IsValid() && $Sender->ShowOptions) {
        ...
        $Sender->Options .= '<li>' . Anchor('<i class="icon-star ' . ($Discussion->Bookmarked == '1' ? '' : ' icon-50 ') . '"></i> ' . ($Discussion->Bookmarked == '1' ? 'Из закладок' : 'В закладки') . '', '/vanilla/discussion/bookmark/' . $Discussion->DiscussionID . '/' . $Session->TransientKey() . '?Target=' . urlencode($Sender->SelfUrl), '' . ($Discussion->Bookmarked == '1' ? '' : ''), array('title' => $Title)
        ) . '</li>';
        ...
        if ($Sender->Options != '') {
            ...
            <div class="dropdown pull-right" >
                <a class="dropdown-toggle" id="dLabel"  data-toggle="dropdown" data-target="#" href="/page.html">
                    <i class="icon-cog label"></i>
                    <b class="caret"></b>
                </a>
